...
static int rtnl_addr_build(Err* err, struct rtnl_link* link, const char* cidr, int ifidx, struct rtnl_addr** out) {
	int nl_err;
	const char* slash = strchr(cidr, '/');
	if (!slash) {
		fprintf(stderr, "invalid CIDR (missing /): %s\n", cidr);
		// todo: free resources
		ERRF(err, "Invalid CIDR (missing /)", "%s", cidr);
		return 1;
	}
	...
	if (ip_len >= sizeof(ip_only)) {
		...
		// todo: free resources
		ERRF(err, "Invalid CIDR (IP too long)", "%s", cidr);
		return 1;
	}
	memcpy(ip_only, cidr, ip_len);
	ip_only[ip_len] = '\0';
	...
	if (*endp != '\0' || pref < 0 || pref > 32) {
		...
		// todo: free resources
		ERRF(err, "Invalid CIDR prefix", "%s", cidr);
		return 1;
	}
...
static int add_routing_rule(Err* err, struct nl_sock* sk, const char* cidr, const char* next_ip, int via_ifidx) {
	int nl_err;
	...
	rtnl_route_set_protocol(route, RTPROT_STATIC);   /* or RTPROT_BOOT */
	rtnl_route_set_scope(route, RT_SCOPE_UNIVERSE);
	rtnl_route_set_type(route, RTN_UNICAST);

	struct nl_addr* nl_addr_cidr = NULL;
	if ((nl_err = nl_addr_parse(cidr, AF_INET, &nl_addr_cidr))) {
		fprintf(stderr, "nl_addr_parse failed for %s: %s\n", cidr, nl_geterror(nl_err));
		// todo: free resources
		ERRF(err, "Nl_addr_parse failed for %s", "%s", cidr, nl_geterror(nl_err));
		return 1;
	}

	struct nl_addr* nl_addr_next_ip = NULL;
	if ((nl_err = nl_addr_parse(next_ip, AF_INET, &nl_addr_next_ip))) {
		fprintf(stderr, "nl_addr_parse failed for %s: %s\n", next_ip, nl_geterror(nl_err));
		// todo: free resources
		ERRF(err, "Nl_addr_parse failed for %s", "%s", next_ip, nl_geterror(nl_err));
		return 1;
	}

	if (nl_err = rtnl_route_set_dst(route, nl_addr_cidr) < 0) {
		fprintf(stderr, "failed to set route addr: %s\n", nl_geterror(nl_err));
		// todo: free resources
		ERRF(err, "Failed to set route addr", "%s", nl_geterror(nl_err));
		return 1;
	}
	...
	if (nl_err = rtnl_route_add(sk, route, NLM_F_CREATE | NLM_F_REPLACE)) {
...
static int create_bridge(Err* err, struct nl_sock* sk, const char* bridge_cidr) {
	int nl_err;
	int existing_ifidx = if_nametoindex(HOST_BRIDGE_NAME);
	if (existing_ifidx != 0) {
		fprintf(stderr, "bridge already exists (ifidx=%d)\n", existing_ifidx);
		return 0;
	}
	...
	if (rtnl_addr_build(err, bridge_link, bridge_cidr, if_nametoindex(HOST_BRIDGE_NAME), &raddr)) {
...
static int create_vxlan(Err* err, struct nl_sock* sk, const char* host_physical_if) {
	...
	struct nl_addr* local = NULL;
	if ((nl_err = nl_addr_parse(VXLAN_GROUP, AF_INET, &local))) {
	...
	rtnl_link_set_name(vxlan_link, HOST_VXLAN_NAME);
	rtnl_link_vxlan_set_id(vxlan_link, VXLAN_VNI_ID);
...
static int configure_container_veth(Err* err, int container_netns_fd, const char* container_veth_name,
	...
	struct rtnl_addr* raddr;
	if (rtnl_addr_build(err, link, container_veth_cidr, ifidx, &raddr)) {
...
static int attach_ifs_to_bridge(Err* err, struct nl_sock* sk, const char* host_veth_name) {
	...
	// fetches a reference (rtnl_link) to the bridge interface from kernel
	if (nl_err = rtnl_link_get_kernel(sk, 0, HOST_VXLAN_NAME, &vxlan_link)) {
	...
	if (rtnl_link_change(sk, vxlan_link, changes_link, 0)) {
...
int network_attach_container(Err* err, const char* container_netns_name, const char* container_netif_name,
	...
	if (create_bridge(err, sk, bridge_cidr)) {
	...
	if (create_veth(err, sk, container_netns_fd, container_netif_name, container_if_tmp_name, host_if_name, container_netif_cidr, bridge_cidr)) {
